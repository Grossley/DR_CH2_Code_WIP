var _temp_local_var_4, _temp_local_var_46, _temp_local_var_48;
if (init == 0)
{
...
if (type == 0 || type == 9 || type == 8)
{
    ...
    {
        bc = gml_Script_instance_create(obj_growtangle.x, (gml_Script_cameray() + 60), obj_queen_buffercontroller)
        if (type == 0)
        {
            global.turntimer += 40
            special = 4
    ...
    if (special > 1 && made <= 0 && (btimer >= 7 || (special == 4 && btimer >= 3)))
    {
        ...
        else if (special == 4)
        {
            ...
            else
            {
                ...
                if (yy > (obj_growtangle.y - 130))
                    xx = (obj_growtangle.x + (random_range(200, 250) * gml_Script_randomsign()))
                else
                    xx = (obj_growtangle.x + (irandom(250) * gml_Script_randomsign()))
            }
        }
        else if (special == 5)
        {
        ...
        else
        {
            ...
            if (made == 0)
            {
                xx = (obj_growtangle.x + (gml_Script_randomsign() * 60))
    ...
    if (special == 4)
    {
        d = gml_Script_instance_create(xx, yy, obj_queen_search_window)
        if (made == side)
            d.search = 1
        else if (made > 0)
            d.search = 2
    ...
    if (init == 3 || init == 4)
    {
        if (init == 3)
        {
            ...
            with (obj_heartmarker)
                instance_destroy()
            gml_Script_instance_create((obj_growtangle.x - 8), (obj_growtangle.y + 30), obj_heartmarker)
...
else if (type == 4)
{
    ...
    if (init == 1)
    {
        ...
        d = gml_Script_instance_create((gml_Script_camerax() + 660), (gml_Script_cameray() + 500), obj_queen_finger)
        if (difficulty == 2)
    ...
    if (btimer >= 10)
    {
        var spawnheight = ((gml_Script_cameray() - 40) + (4 * (btimer - 10)))
        ...
        if (special != 5)
        {
            d = gml_Script_instance_create((minx - 40), spawnheight, obj_queen_pfp)
            d.image_speed = 0
            d.direction = 270
            d.speed = 4
...
else if (type == 6 || type == 6.1)
{
    ...
    if (btimer > 30)
    {
        if bufferattack
        {
            d = gml_Script_instance_create((obj_growtangle.x + 150), obj_growtangle.y, obj_queen_explodinghead)
            d.damage = damage
            d.target = target
        ...
        {
            if (irandom(100) < 60)
                d = gml_Script_instance_create((obj_growtangle.x + (lastside * 150)), ((obj_growtangle.y - 70) + irandom(140)), obj_queen_explodinghead_intro)
            else
                d = gml_Script_instance_create(((obj_growtangle.x - 70) + irandom(140)), (obj_growtangle.y + (lastside * 150)), obj_queen_explodinghead_intro)
            lastside *= -1
        }
        d.buffer = bufferattack
        d.damage = damage
...
else if (type == 100)
{
    if (btimer >= 15)
    {
        ...
        gun = gml_Script_instance_create((cenx + lengthdir_x(len, dir)), (ceny + lengthdir_y(len, dir)), obj_queen_lasergun)
        btimer = 0
    }
}
else if (type == 101 || type == 102)
{
    ...
    if (btimer >= btimerthreshold)
    {
        ...
        if (goway == 0)
        {
            for (i = 0; i < 4; i++)
            {
                gun = gml_Script_instance_create((fromx - ((xway * 50) * i)), (ceny + (125 * yway)), obj_queen_lasergun)
                gun.inittime = (i * 4)
                gun.randomshot = 0
                gun.shotsize = 0.5
                gun.maxsize = 1
        ...
        if (goway == 1)
        {
            for (i = 0; i < 4; i++)
            {
                gun = gml_Script_instance_create((cenx + (125 * xway)), (fromy - ((yway * 50) * i)), obj_queen_lasergun)
                gun.inittime = (i * 4)
                gun.randomshot = 0
                gun.shotsize = 0.5
                gun.maxsize = 1
...
else if (type == 105 || type == 106 || type == 107)
{
    if (!instance_exists(obj_social_media_attack_fade_heroes))
        gml_Script_instance_create(x, y, obj_social_media_attack_fade_heroes)
    timerthreshold = 15
    if (type == 106)
        timerthreshold = 20
    init++
    ...
    if (btimer >= timerthreshold)
    {
        ...
        if make
        {
            side = choose(-1, 1)
            if (type == 107)
                face = gml_Script_instance_create((obj_growtangle.x + (160 * side)), -80, obj_socialmedia_avatar)
            else
                face = gml_Script_instance_create((obj_growtangle.x + (125 * side)), -80, obj_socialmedia_avatar)
            face.vspeed = 5
        }
        if (made == 4 && type == 107)
        {
            face = gml_Script_instance_create((obj_growtangle.x + 125), -80, obj_socialmedia_avatar)
            face.vspeed = 5
            face.isqueen = 1
            face.side = 1
...
else if (type == 110 || type == 111)
{
    ...
    {
        growtangle = gml_Script_instance_create(obj_growtangle.x, obj_growtangle.y, obj_growtangle_electric)
...
else if (type == 112 || type == 113)
{
    if (made == 0 && instance_exists(obj_growtangle))
    {
        ...
        bardly = gml_Script_instance_create(obj_growtangle.x, (obj_growtangle.y - obj_growtangle.sprite_height), obj_berdly_tornadomaker)
        if (type == 113)
            bardly.difficulty = 3
        made = 1
    }
