var _temp_local_var_4, _temp_local_var_40;
if (partmode == 10)
    partmode = 9
...
if (gml_Script_i_ex(355) && global.mercymod[myself] > 89 && bgvinecount > 0 && (!gml_Script_i_ex(686)) && (!gml_Script_i_ex(685)))
{
    for (i = 0; i < 6; i += 1)
    {
        if (obj_spamton_neo_enemy.partvisible[i] == 0)
        {
            ...
            vine = gml_Script_instance_create(x, y, obj_sneo_vine_transition)
        }
    }
}
if (specialcon == 1)
...
for (i = 0; i < 8; i += 1)
{
    ...
    if (partmode >= 30)
    {
        ...
        if (partmode == 31 && partmode < 34)
        {
            if (i == 5)
            {
                ...
                repeat (5)
                {
                    var eyeshine = gml_Script_instance_create((((((_n * 8) + x) + (partx[5] * facing)) + (partxoff[5] * facing)) + lastxoff), ((y + party[5]) + partyoff[5]), obj_afterimage_grow)
    ...
    if (partmode == 34)
    {
        ...
        if (i == 0)
        {
            ...
            if makeheart
            {
                ...
                if (hearttype == 0)
                {
                    wireheart = gml_Script_instance_create((x + 10), (y + 80), obj_sneo_wireheart)
                    wireheart.boss = id
                ...
                for (_n = 0; _n < 6; _n++)
                {
                    var shard = gml_Script_instance_create((x + 10), (y + 80), obj_afterimage)
        ...
        if (i == 0 && endcon == 0)
        {
            ...
            if (dance_timer == (42 - (fastshot * 10)))
            {
                with (obj_sneo_biglaser)
                    instance_destroy()
                shot = gml_Script_instance_create(armendx, armendy, obj_spamtonshot)
            ...
            {
                chargepitch = 0.1
                gml_Script_snd_pitch(chargeshot_sound, chargepitch)
                gml_Script_snd_volume(chargeshot_sound, 0, 0)
                gml_Script_snd_volume(chargeshot_sound, 0.8, 20)
            }
    ...
    if (i == 0)
    {
        if (keyboard_check_released(ord("O")) && gml_Script_scr_debug())
        {
            partmode = 43
            y = ystart
            fakegrav = 0
    ...
    if (partmode == 42)
    {
        ...
        if (i == 0)
        {
            ...
            if (fallen == 1)
            {
                if (y < (gml_Script_cameray() + 216))
                {
                    partrot[2] = lerp(partrot[2], -90, 0.1)
                    partrot[3] = lerp(partrot[3], 90, 0.1)
                    partrot[5] = (y - (gml_Script_cameray() + 120))
                    if (y > ((gml_Script_cameray() + 216) - 30))
                    {
                        partrot[1] = lerp(partrot[1], -30, 0.1)
                        partrot[6] = lerp(partrot[6], 30, 0.1)
                    }
                    y = lerp(y, (gml_Script_cameray() + 216), 0.1)
                }
            }
            if (fallen == 0)
            {
    ...
    gml_Script_draw_monster_body_part_ext(partsprite[i], partframe[i], ((((x + (partx[i] * facing)) + (partxoff[i] * facing)) + lastxoff) + shakevar), (((y + party[i]) + partyoff[i]) - shakevar), ((2 + scalebonus) * facing), (2 + scalebonus), (partrot[i] * facing), partblend[i], image_alpha)
    if (violentendflash > 0)
    {
        gml_Script_d3d_set_fog(true, c_white, 0, 1)
        gml_Script_draw_monster_body_part_ext(partsprite[i], partframe[i], ((((x + (partx[i] * facing)) + (partxoff[i] * facing)) + lastxoff) + shakevar), (((y + party[i]) + partyoff[i]) - shakevar), ((2 + scalebonus) * facing), (2 + scalebonus), (partrot[i] * facing), partblend[i], (violentendflash / 10))
