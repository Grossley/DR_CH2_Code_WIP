...
if (state < 2)
{
    ...
    if destroyable
    {
        ...
        _box.timer = gml_Script_scr_movetowards(_box.timer, (clamp(gml_Script_scr_inverselerp(520, (obj_growtangle.x + 100), x), 0, 1) * 60), 2)
    ...
    if ((destroyable && state == 0) || (state == 1 && formtimer < 30))
    {
        if (inhalesfx == 0)
        {
            ...
            gml_Script_snd_pitch_time(snd_sneo_overpower, 1, 90)
    ...
    if ((state == 0 && x < (obj_growtangle.x - (obj_growtangle.sprite_width / 2.5))) || (state == 0 && obj_heart.x > (x + 70)))
    {
        ...
        fakeheart = gml_Script_instance_create(obj_heart.x, obj_heart.y, obj_sneo_fakeheart)
    }
...
else if (state == 3)
{
    if (movecon == 0)
    {
        ...
        image_angle = gml_Script_lerp_ease_in(image_angle, 25, movetimer, 2)
        if (movetimer == 1)
        {
            movetimer = 0
            movecon = 1
    ...
    if (movecon == 2)
    {
        ...
        {
            movetimer += (1 / moveframes)
            image_angle = gml_Script_lerp_ease_in(image_angle, -1, movetimer, 2)
    ...
    if (timer2 < 100)
        timer2++
    x = lerp(x, (gml_Script_camerax() + 439), (timer2 / 100))
    if (x > (obj_growtangle.x + 70))
        obj_sneo_warped_box.state = 2
    else
    {
        ...
        _box.suckx = min((obj_growtangle.x + 150), (x + 106))
        _box.timer = (clamp(gml_Script_scr_inverselerp((gml_Script_camerax() + 520), (obj_growtangle.x + 100), x), 0, 1) * 60)
        _box.upper_sucky = (y + 129)
        _box.lower_sucky = (y + 159)
    }
    if (x > (gml_Script_camerax() + 438))
    {
        x = (gml_Script_camerax() + 438)
        movecon = 2
...
else if (state == 4 && formtimer >= 25)
{
    formtimer = 0
    state = 5
    gml_Script_instance_create((x + mouthx), (y + mouthy), obj_sneo_final_orb)
    image_index = 5
}
else if (state == 10)
{
...
else if (state == 11)
{
    ...
    if (x < (gml_Script_camerax() + 438))
        x += 4
    if (x > (gml_Script_camerax() + 438) && formtimer < 65)
        formtimer = 65
    ...
    if (x > (gml_Script_camerax() + 438))
    {
        ...
        fakeheart.state = 2
        image_index = 5
        gml_Script_scr_debug_print("spitting heart")
    }
}
else if (state == 12)
{
    ...
    if (formtimer >= 30 && movetimer >= 1)
    {
        ...
        image_index = 4
        instance_destroy()
        gml_Script_instance_create(x, (ystart - abs((sin((steptimer + 0.15)) * 40))), obj_sneo_lastattack)
    }
}
if (state >= 11)
{
