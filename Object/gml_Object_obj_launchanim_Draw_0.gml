if (t == 0)
{
    shakex = gml_Script_scr_movetowards(shakex, 0, 1)
    shakex *= -1
    hurtshake = 0
    draw_sprite_ext(sprite_index, 0, (x + shakex), y, 2, 2, 0, image_blend, 1)
}
else
    gml_Script_draw_sprite_center_pivot(sprite_index, image_index, x, y)
sweat = 0
if (g <= 5)
    sweat = 1
if (g >= 9 && g <= 13)
...
if ((t > 0 && t <= 20) || g == 19)
{
    ...
    d.image_xscale = ((choose(1, 1) * (gml_Script_scr_remapvalue(1, 10, t, 3, 1) + random_range(-0.2, 0.2))) - _xscale)
    d.image_yscale = ((choose(1, 1) * (gml_Script_scr_remapvalue(1, 10, t, 3, 1) + random_range(-0.2, 0.2))) - _yscale)
    d.gravity = -0.1
