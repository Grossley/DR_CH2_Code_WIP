if (init == 0)
{
    ...
    {
        boys[0].x += 16
        boys[1] = gml_Script_scr_childbullet((x - 16), y, 420)
    ...
    {
        boys[0].x += 32
        boys[1] = gml_Script_scr_childbullet(x, y, 420)
        boys[2] = gml_Script_scr_childbullet((x - 32), y, 420)
        boys[1].target = targetB
        boys[1].grazepoints = grazepoints
...
if ((shottimer <= 0 && y < (obj_growtangle.y + 50)) || force_shot)
{
    ...
    if gml_Script_i_ex(boys[shootingboy])
    {
...
for (i = 0; i < fleetsize; i++)
{
    ...
    else if ((timer + ((i + 1) * 2)) == moveinterval)
    {
        ...
        if (!gml_Script_i_ex(boys[target]))
        {
        }
        else
        {
    ...
    else if (!gml_Script_i_ex(boys[i]))
    {
    }
    else
        boys[i].y = y
