if (id == instance_find(obj_sneo_faceattack, 0))
{
    ...
    eyes = gml_Script_instance_create(x, y, obj_sneo_faceattack)
    eyes.hp = eyeshp
    eyes.type = 1
    eyes.target = target
    eyes.damage = damage
    eyes.first_time = first_time
    nose = gml_Script_instance_create(x, y, obj_sneo_faceattack)
    nose.hp = nosehp
    nose.type = 2
    nose.target = target
    nose.damage = damage
    nose.first_time = first_time
    mouth = gml_Script_instance_create(x, y, obj_sneo_faceattack)
    mouth.hp = mouthhp
    mouth.type = 3
    mouth.target = target
    mouth.damage = damage
