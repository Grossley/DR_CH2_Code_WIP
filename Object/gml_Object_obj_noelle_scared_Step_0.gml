...
if (con == 2 && jumpstyle == 1)
{
    ...
    if (image_index >= 5)
        image_index = 3
    if ((y + jumpy) > (gml_Script___view_get(1, 0) - 80))
        jumpy += jumpyspeed
    else
    {
        x += 10
...
if (fallen == 1)
{
    x = 114
    y = (room_height + 20)
    tower = gml_Script_instance_create(x, y, obj_tower_o_mice)
    gml_Script_scr_move_to_point_over_time(115, 285, 60)
    fallen = 2
    timer = 0
}
if (fallen == 2)
...
{
    noelleFacing = gml_Script_instance_create(x, y, obj_npc_facing)
