...
if (make_effect_timer >= 10)
{
    for (i = 0; i < 10; i++)
    {
        if (actor_visible[i][step_current] == 1)
        {
            ...
            {
                fake_actor = gml_Script_instance_create(actor_startx[i][step_current], actor_starty[i][step_current], obj_actor)
                ...
                mover = gml_Script_instance_create(0, 0, obj_move_to_point)
...
for (i = 0; i < 10; i++)
{
    if (actor_visible[i][step_current] == 1)
    {
        ...
        var _temp_local_var_4 = mouse_y.mouse_x
    }
}
for (j = 0; j <= step_max; j++)
{
    for (i = 0; i < 10; i++)
    {
        if (actor_visible[i][j] == 1)
        {
            ...
            draw_text(actor_endx[i][j], actor_endy[i][j], string_hash_to_newline(gml_Script_stringsetsubloc("actor, ~1  step:~2", string(i), string(j), "obj_cutscene_maker_slash_Draw_0_gml_60_0")))
        }
    }
}
