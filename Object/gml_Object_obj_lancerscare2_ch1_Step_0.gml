...
if (con == 11 && gml_Script_d_ex_ch1() == 0)
{
    ...
    image_speed = 0.5
    bucket = gml_Script_scr_dark_marker_ch1((x + 20), y, spr_lancer_bucket_ch1)
    bucket.hspeed = -4
    bucket.depth = (depth + 10)
    gml_Script_snd_free_all_ch1()
    gml_Script_snd_play_ch1(snd_lancerlaugh_ch1)
    gml_Script_snd_play_ch1(snd_locker_ch1)
    lightning = gml_Script_scr_dark_marker_ch1(200, 0, spr_crappylightning_ch1)
...
{
    fd = gml_Script_instance_create_ch1(0, 0, obj_fadeout_ch1)
    con = 64
    alarm[4] = 40
}
if (con == 65)
...
if (con == 66)
{
    with (fd)
        instance_destroy()
    fi = gml_Script_instance_create_ch1(0, 0, obj_fadein_ch1)
    con = 67
    alarm[4] = 30
}
if (con == 68)
