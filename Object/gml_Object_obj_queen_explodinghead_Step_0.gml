if (global.turntimer < 2)
    instance_destroy()
if (laugh && gml_Script_scr_queen_buffercheck())
{
    if (laughpause == 0 && gml_Script_snd_is_playing(laughsound))
    {
        laughpause = 1
        gml_Script_snd_pause(laughsound)
    }
    return;
}
...
{
    laughpause = 0
    gml_Script_snd_resume(laughsound)
}
if (timer == 20)
{
    image_speed = 1
...
{
    var bc = gml_Script_instance_create(0, 0, obj_queen_buffercontroller)
    bc.bufferdelay = 15
...
if (timer == 0)
{
    ...
    if (ultimate == 1)
    {
        for (i = 0; i < 10; i++)
        {
            ...
            d = gml_Script_instance_create((x + lengthdir_x(_shotspread, _shotdir)), (y + lengthdir_y(_shotspread, _shotdir)), _bullet)
    ...
    d = gml_Script_instance_create(x, y, obj_buffer_tempeffect)
