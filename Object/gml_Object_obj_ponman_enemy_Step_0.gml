...
if (global.myfight == 3)
{
    ...
    if (actcon == 11 && instance_exists(obj_writer) == 0)
    {
        ...
        with (obj_heroralsei)
            visible = true
        gml_Script_snd_stop(singy)
        gml_Script_snd_resume(global.batmusic[1])
        global.msg[0] = gml_Script_stringsetloc("* PONMAN fell asleep^1!&* The enemies became TIRED!/%", "obj_ponman_enemy_slash_Step_0_gml_186_0")
        if (gml_Script_scr_monsterpop() > 1 && gml_Script_scr_havechar(2))
        {
            for (dx = 0; dx < 3; dx += 1)
            {
                if (global.char[dx] == 2)
                {
                    if (global.charcond[dx] != 5)
                    {
                        ...
                        global.msg[0] = gml_Script_stringsetloc("* PONMAN fell asleep^1!&* SUSIE fell asleep^1!&* The enemies became TIRED!/%", "obj_ponman_enemy_slash_Step_0_gml_198_0")
                    }
                }
            }
        }
