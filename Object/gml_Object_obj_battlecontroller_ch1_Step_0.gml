if (victory == true && victoried == 0)
{
    ...
    if (skipvictory == false)
    {
        ...
        global.fc = 0
        global.fe = 0
        global.battlemsg[0] = gml_Script_scr_84_get_subst_string_ch1(gml_Script_scr_84_get_lang_string_ch1("obj_battlecontroller_slash_Step_0_gml_40_0"), string(global.monsterexp[3]), string(global.monstergold[3]))
        global.msg[0] = global.battlemsg[0]
        global.typer = global.battletyper
        lastbattlewriter = gml_Script_scr_battletext_ch1()
        for (i = 0; i < 3; i += 1)
        {
...
if (global.myfight == 0)
{
    ...
    if (global.bmenuno == 2)
    {
        ...
        if (gml_Script_button1_p_ch1() && global.spell[thischar][global.bmenucoord[2][global.charturn]] != 0 && onebuffer < 0)
        {
            if (global.spellcost[thischar][global.bmenucoord[2][global.charturn]] <= global.tension)
            {
                ...
                global.bmenuno = 0
                selnoise = true
                gml_Script_scr_spellinfo_ch1(global.spell[thischar][global.bmenucoord[2][global.charturn]])
                if (spelltarget == 0)
                    gml_Script_scr_spellconsumeb_ch1()
                if (spelltarget == 1)
                    global.bmenuno = 8
                if (spelltarget == 2)
                    global.bmenuno = 3
    ...
    if (global.bmenuno == 4)
    {
        ...
        if (tempitem[global.bmenucoord[4][global.charturn]][global.charturn] == 0)
            global.bmenucoord[4][global.charturn] -= 1
        if (gml_Script_button1_p_ch1() && tempitem[global.bmenucoord[4][global.charturn]][global.charturn] != 0 && onebuffer < 0)
        {
            onebuffer = 2
            global.bmenuno = 0
            selnoise = true
            gml_Script_scr_iteminfo_ch1(tempitem[global.bmenucoord[4][global.charturn]][global.charturn])
            if (itemtarget == 0 || itemtarget == 2)
                gml_Script_scr_itemconsumeb_ch1()
            if (itemtarget == 1)
                global.bmenuno = 7
        }
        if (gml_Script_button2_p_ch1() && onebuffer < 0)
