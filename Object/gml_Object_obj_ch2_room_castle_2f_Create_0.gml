...
else if (global.plot >= 200 && global.flag[387] == 1)
{
    ...
    else if (gml_Script_scr_get_total_recruits(2) >= 9)
    {
        ...
        with (queen_npc)
            gml_Script_scr_depth()
        var lancer_npc = gml_Script_instance_create(875, 235, obj_npc_sign)
        lancer_npc.sprite_index = spr_lancer_dark_relax
        lancer_npc.image_speed = 0
        with (lancer_npc)
            gml_Script_scr_depth()
        var rudinn_npc = gml_Script_instance_create(1120, 240, obj_npc_room_animated)
