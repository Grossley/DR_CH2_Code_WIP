...
if (cancelattack == false)
{
    ...
    if (damage > 0)
    {
        if (global.monstertype[0] != 20)
            gml_Script_scr_tensionheal(round((points / 10)))
        if (global.monstertype[0] == 20)
            gml_Script_scr_tensionheal(round((points / 15)))
        attack = gml_Script_instance_create((global.monsterx[global.chartarget[myself]] + random(6)), (global.monstery[global.chartarget[myself]] + random(6)), obj_basicattack)
