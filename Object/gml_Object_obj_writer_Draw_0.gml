...
button3 = 0
miniface_drawn = -1
if (gml_Script_button1_p() == 1 && prevent_mash_buffer <= 0)
    button1 = 1
if (gml_Script_button2_h() == 1 && prevent_mash_buffer <= 0)
    button2 = 1
if (global.flag[10] == 1 || gml_Script_scr_debug())
{
    if (gml_Script_button3_h() == 1)
    {
...
if (dialoguer == 1 && formatted == 0)
{
    ...
    if instance_exists(obj_dialoguer)
    {
        ...
        {
            writingx = (gml_Script_camerax() + obj_dialoguer.remwriterx)
            if (global.fc > 0)
                writingx = ((gml_Script_camerax() + obj_dialoguer.remwriterx) + (58 * f))
        }
    }
}
if (formatted == 0)
{
    ...
    if (autocenter == 1)
    {
        x = (((gml_Script_camerax() + (gml_Script_camerawidth() / 2)) - ((stringmax * hspace) / 2)) + 5)
        y = (((gml_Script_cameray() + (gml_Script_cameraheight() / 2)) - ((writingy + ((linecount + 1) * vspace)) / 2)) - 10)
    }
    if (charpos > stringmax)
        stringmax = charpos
    formatted = 1
...
for (n = 1; n < pos; n += 1)
{
    ...
    else if (mychar == "\\")
    {
        ...
        if (nextchar == "f" && faced == 0)
        {
            fam = 0
            fam = real(nextchar2)
            if (!gml_Script_i_ex(global.sminstance[fam]))
            {
                global.sminstance[fam] = gml_Script_instance_create(global.smxx[fam], global.smyy[fam], obj_smallface)
                smallface = global.sminstance[fam]
                if gml_Script_i_ex(smallface)
                {
                    smallface.x += x
                    smallface.y += y
                    ...
                    var _temp_local_var_28 = global.smalarm[fam]
                }
            }
        }
        if (nextchar == "*")
        {
            wx = round(wx)
            var _sprite = gml_Script_scr_getbuttonsprite(nextchar2, 1)
            var y_offset = 0
            var x_offset = 0
        ...
        if (nextchar == "c")
        {
            ...
            if (nextchar2 == "M")
                xcolor = c_maroon
            if (nextchar2 == "S")
                xcolor = gml_Script_hexcolor(16744703)
            if (nextchar2 == "V")
                xcolor = gml_Script_hexcolor(8454016)
            if (nextchar2 == "0")
                xcolor = mycolor
        }
        if (nextchar == "C")
        {
            if (nextchar2 == "1")
            {
                if (instance_exists(obj_choicer_old) == 0)
                    choicer = gml_Script_instance_create(0, 0, obj_choicer_old)
                halt = 5
            }
            if (nextchar2 == "2" || nextchar2 == "3" || nextchar2 == "4")
            {
                ...
                {
                    choicer = gml_Script_instance_create(0, 0, obj_choicer_neo)
        ...
        if (nextchar == "S")
        {
            if (sound_played == false)
            {
                for (i = 0; i < 10; i += 1)
                {
                    ...
                    {
                        gml_Script_snd_play(global.writersnd[i])
                        sound_played = true
                    }
                }
            }
    ...
    if (accept == true)
    {
        ...
        if (jpspecial == 1)
        {
            if gml_Script_scr_kana_check(mychar)
            {
