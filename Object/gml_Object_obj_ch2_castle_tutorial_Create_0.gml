...
if (global.chapter == 2 && global.plot >= 9)
{
    ...
    var elegance = gml_Script_instance_create(494, 86, obj_npc_room)
    ...
    if (global.plot >= 200)
    {
        ...
        var mannequin = gml_Script_instance_create(834, 150, obj_npc_room)
        mannequin.sprite_index = spr_mannequin_ralsei
        with (mannequin)
            gml_Script_scr_depth()
        var addison = gml_Script_instance_create(900, 156, obj_npc_room)
        ...
        if (gml_Script_scr_get_total_recruits(2) >= 9)
        {
            ...
            var music_shop_solid2 = gml_Script_instance_create(710, 120, obj_solidblocksized)
            music_shop_solid2.image_xscale = 1.675
            music_shop_solid2.image_yscale = 1.3
            var music_door = gml_Script_instance_create((music_shop.x + 35), (music_shop.y + 120), obj_doorw_musfade)
            music_door.image_xscale = 4
            music_door.image_yscale = 2
        }
        else
        {
            sweetnpc = gml_Script_instance_create(598, 41, obj_npc_room_animated)
            sweetnpc.sprite_index = spr_npc_sweet
            with (sweetnpc)
                gml_Script_scr_depth()
            capnnpc = gml_Script_instance_create(670, 74, obj_npc_room_animated)
            capnnpc.sprite_index = spr_npc_hatguy
            with (capnnpc)
                gml_Script_scr_depth()
            kknpc = gml_Script_instance_create(743, 28, obj_npc_room_animated)
