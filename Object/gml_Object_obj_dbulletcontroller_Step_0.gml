var _temp_local_var_1, _temp_local_var_12, _temp_local_var_13, _temp_local_var_16, _temp_local_var_120, _temp_local_var_122, _temp_local_var_123, _temp_local_var_124, _temp_local_var_134, _temp_local_var_167, _temp_local_var_168, _temp_local_var_169;
if (init == 0)
{
...
if (type == 0)
{
    if (btimer >= (timermax * ratio))
    {
        ...
        bm = gml_Script_instance_create(((obj_heart.x + 8) + xx), ((obj_heart.y + 8) + yy), obj_dbullet_maker)
...
if (type == 1)
{
    if (btimer >= (9 * ratio))
    {
        ...
        if (num == 3)
            xx = (-10 + random(20))
        d = gml_Script_instance_create(((obj_heart.x + 8) + xx), ((obj_heart.y + 8) + yy), obj_dbullet_vert)
        d.damage = damage
        d.target = target
    }
}
else if (type == 2)
{
    if (btimer >= (10 * ratio))
    {
        ...
        bm = gml_Script_instance_create(x, y, obj_yarnmaker)
        gml_Script_scr_bullet_inherit(bm)
        var childgravity = 0.5
        var launchspeed = gml_Script_scr_remapvalue(40, 200, (y - gml_Script_cameray()), 8, 16)
        if ((made % 3) == 2)
            bm.direction = (180 - gml_Script_scr_getlaunchdirection(launchspeed, ((x - obj_heart.x) + 4), childgravity, 1))
        else
            bm.direction = (180 - gml_Script_scr_getlaunchdirection(launchspeed, (random(250) + (x - maxx)), childgravity, 1))
        bm.damage = damage
        bm.target = target
        bm.childgravity = 0.6
...
else if (type == 4 && (sameattack < 3 || creator < obj_84_lang_helper))
{
    if (btimer > 7)
    {
        ...
        xx = (x - (made * 30))
        yy = maxy
        d = gml_Script_instance_create(xx, yy, obj_chainbullet)
        d.damage = damage
        d.grazepoints = 4
        d.direction = 90
        d.childSpeed = 7
...
else if (type == 5)
{
    if (init == 1)
    {
        ...
        if (obj_swatchling_battle_controller.yellow_count > 0)
        {
            ...
            obj_swatchling_battle_controller.bird_color[_birdcount] = (special == 1 ? 4235519 : 255)
...
else if (type == 9)
{
    ...
    {
        d = gml_Script_instance_create((x + 24), (y + 30), obj_berdlyb_spearblast)
        d.grazepoints = 2
        d.childgraze = 1
        btimer = 10
        d.timepoints = 0.5
...
else if (type == 10)
{
    ...
    {
        d = gml_Script_instance_create((x + 46), (y + 40), obj_berdlyb_chirashistorm)
        d.target = target
        d.damage = damage
        d.difficulty = difficulty
        d.grazepoints = 4
        if (made > 0 && gml_Script_scr_monsterpop() == 1)
            d.first_set = 0
        made++
        btimer = 0
        if (difficulty < 1)
...
else if (type == 11)
{
    ...
    if (btimer >= (_btimeSpeed * ratio))
    {
        ...
        d = gml_Script_instance_create(xx, yy, obj_poppup_bird)
        d.speed = (special ? 3 : 5)
        if special
        {
            d.slow = 0.6
...
else if (type == 12 && sameattacker == 0)
{
    ...
    if (btimer >= (sameattack == 2 ? 15 : 20))
    {
        ...
        d = gml_Script_instance_create(xx, yy, obj_poppup_ad)
        d.depth -= 15
        if special
        {
            d.slow = 1
...
else if (type == 13)
{
    ...
    {
        side = irandom(2)
        monstercount = gml_Script_scr_monsterpop()
        yoffset = 5
        if (monstercount != sameattack)
            yoffset += 20
        made = 1
        sidedirection = ((irandom(1) * 2) - 1)
        cars = (gml_Script_scr_monsterattacknamecount("CarChase") > 0 || gml_Script_scr_monsterattackidcount(16) > 0)
        for (i = 0; i < (monstercount == 1 ? 2 : 3); i++)
        {
            xx = obj_growtangle.x
            if (sameattack == 1)
                xx = (xx + (70 * (-sidedirection)))
            d = gml_Script_instance_create(xx, (miny - yoffset), obj_viro_invaderfleet)
            d.lborder = (minx + 10)
            d.rborder = (maxx - 10)
...
else if (type == 14)
{
    if (btimer >= (ratio == 1 ? 6 : (10 * ratio)))
    {
        ...
        d = gml_Script_instance_create(xx, yy, obj_viro_needle)
        btimer = 0
...
else if (type == 15)
{
    ...
    if (btimer >= (10 * ratio))
    {
        xx = (sameattacker == 0 ? ((minx - 40) - random(140)) : ((maxx + 40) + random(140)))
        yy = (miny + random(obj_growtangle.sprite_height))
        d = gml_Script_instance_create(xx, yy, obj_omawaroid_vaccine)
        d.grazepoints = 4
        d.direction = (sameattacker == 0 ? 0 : 180)
        d.damage = damage
        d.target = target
        d.speed = (gml_Script_scr_monsterpop() == 1 ? 7 : 5)
        btimer = 0
    }
}
else if (type == 16)
{
    if (init == 1)
    {
        ...
        {
            street = gml_Script_instance_create(obj_growtangle.x, obj_growtangle.y, obj_omawaroid_street)
            if (special != 0)
    ...
    {
        d = gml_Script_instance_create((obj_growtangle.x + ((40 + random(20)) * side)), (miny - 30), obj_omawaroid_cactus)
        if (sameattack == 1)
            side *= -1
        d.direction = 270
        d.speed = 6
        ...
        if (made == 2)
        {
            ...
            d = gml_Script_instance_create((obj_growtangle.x + (xoffset * carside)), (maxy + 26), obj_omawaroid_policecar)
            d.direction = 90
            d.speed = (irandom(2) + 2)
            if (street.anti_cheese == 1)
                d.speed = (sameattack == 2 ? 3 : 2)
            else if (sameattack == 2 || gml_Script_scr_monsterpop() == 1)
            {
...
else if (type == 17)
{
    if (init == 1)
    {
        ...
        if (sameattacker <= 1)
        {
            d = gml_Script_instance_create((obj_growtangle.x + 1), (obj_growtangle.y + 1), obj_maus_holes)
            d.damage = damage
            d.grazepoints = 4
            d.target = target
        }
...
else if (type == 19)
{
    if (init == 1)
    {
        ...
        d = gml_Script_instance_create(x, y, obj_maus_cursor_follow)
        d.damage = damage
        d.target = target
        d.grazepoints = 4
        d = gml_Script_instance_create(x, y, obj_player_trail)
    ...
    {
        d = gml_Script_instance_create((obj_mauswheel_enemy.x + 62), (obj_mauswheel_enemy.y + 70), obj_maushwheel_lightning_orb)
        d.damage = damage
        d.target = target
        d.grazepoints = 4
    }
...
else if (type == 20)
{
    if (init == 1)
    {
        ...
        d.element = 6
        d.grazepoints = 4
        d.difficulty = gml_Script_scr_monsterpop() == 1
        d.animSpeed = 1
        init = 2
    }
}
...
else if (type == 22)
{
    ...
    if (btimer >= 120)
    {
        ...
        for (i = 0; i < 2; i++)
        {
            d = gml_Script_instance_create(xx, yy, obj_musicalfight_speakers)
            d.direction = (side * 90)
            d.damage = damage
            d.target = target
            d.grazepoints = 4
...
else if (type >= 23 && type <= 25)
{
    if (init == 1)
    {
        ...
        btimer = -10
        if (type == 25)
            gml_Script_instance_create(obj_growtangle.x, obj_growtangle.y, obj_spamton_warped_box)
        if (type == 23)
        {
            d.bullettype = 583
            d.firingspeed = 10
    ...
    else if (type == 25 && btimer >= 8 && global.turntimer > 50)
    {
        ...
        d = gml_Script_instance_create(xx, yy, obj_spamton_dollar)
        d.damage = damage
        d.target = target
        d.grazepoints = 4
        d.speed = 0.1
...
else if (type == 26 && init == 1)
{
    ...
    {
        case 0:
            d = gml_Script_instance_create(0, 0, obj_thrash_laserattack)
            d.grazepoints = 8
            d.thrash = _thrash
            d.damage = damage
            break
        case 1:
            d = gml_Script_instance_create(0, 0, obj_thrash_swordattack)
            d.grazepoints = 8
            d.thrash = _thrash
            d.damage = damage
            break
        case 2:
            d = gml_Script_instance_create(0, 0, obj_thrash_flameattack)
            d.grazepoints = 8
            d.thrash = _thrash
            d.damage = damage
            break
        case 3:
            d = gml_Script_instance_create(0, 0, obj_thrash_duck_attack)
            d.grazepoints = 24
            d.thrash = _thrash
            d.damage = 1
            break
...
else if (type == 27)
{
    ...
    switch legattack
    {
        case 0:
            ...
            if ((thrashcon == 3 || thrashcon == 5) && _thrash.stomp != 0)
            {
                for (i = 0; i < 1; i++)
                {
                    if (i == 0)
                    {
                        d = gml_Script_instance_create((x + 30), (gml_Script_gt_maxy() - 36), obj_animation)
                        d.sprite_index = spr_thrash_bash
                        d.image_xscale = choose(3, -3)
                        d.image_yscale = 3
                    }
                    d = gml_Script_instance_create((x + 30), (y + 190), obj_thrash_stomp_bullet)
        ...
        case 1:
            ...
            {
                d = gml_Script_instance_create((x + 30), (y + 190), obj_thrash_wheel_bullet)
                d.direction = random_range(40, 88)
                d.speed = (gml_Script_scr_remapvalue(40, 85, d.direction, 4, 8) + random(3))
                d.bounce = 1
        ...
        case 2:
            ...
            {
                d = gml_Script_instance_create(((x + 13) + (8 * made)), (y + 197), obj_thrash_missiles)
                d.direction = 90
                d.speed = 2
                d.gravity = -0.2
...
else if (type == 28)
{
    if (btimer >= max(10, (15 - (made / 2))))
    {
        ...
        d = gml_Script_instance_create((leftx + (xoffset * 36)), (gml_Script_cameray() - 20), obj_rouxls_fallingblock)
        d.gravity = 0.25
        d.vspeed = 3.5
        d.damage = damage
        d.grazepoints = 8
...
else if (type == 29)
{
    ...
    xx = x
    yy = y
    d = gml_Script_instance_create(xx, yy, obj_thrash_laserbullet)
    d.direction = 0
    d.damage = damage
    d.target = target
    d.grazepoints = 4
...
else if (type == 30)
{
    if (btimer >= (20 * ratio))
    {
        ...
        bul = gml_Script_instance_create(xx, yy, obj_clubsbullet)
        bul.speed = 12
        bul.damage = damage
        bul.target = target
        bul.grazepoints = 4
...
else if (type == 31)
{
    if (btimer >= (30 * ratio))
    {
        ...
        d = gml_Script_instance_create(((obj_heart.x + 8) + xx), ((obj_heart.y + 8) + yy), obj_clubsbullet_dark)
        d.damage = damage
        d.target = target
        d.grazepoints = 4
        d.direction = (dir + 180)
...
else if (type == 32)
{
    if (init == 1)
    {
        ...
        xx = obj_growtangle.x
        yy = obj_growtangle.y
        d = gml_Script_instance_create((xx + 1), yy, obj_bulletparent)
        ...
        for (i = 0; i < 4; i++)
        {
            ...
            if (i == 0 || i == 3)
                ...
                xx += (i == 1 ? -50 : 50)
            d = gml_Script_instance_create(xx, yy, obj_bulletparent)
    ...
    if ((made == 4 || special < -2) && quizReady && btimer >= 0)
    {
        ...
        else if (special == -2)
        {
            ...
            _maru = gml_Script_instance_create(obj_growtangle.x, obj_growtangle.y, obj_tm_quiz_result)
        ...
        {
            if gml_Script_i_ex(dd)
                instance_destroy(dd)
            if gml_Script_i_ex(dd2)
        ...
        else if (special == -3)
        {
            ...
            if (global.encounterno == 89)
            {
                ...
                strikes++
                btimer = -45
                _batsu = gml_Script_instance_create(obj_growtangle.x, obj_growtangle.y, obj_tm_quiz_result)
...
else if (type == 33)
{
    ...
    {
        made = 1
        tail = gml_Script_instance_create((obj_clubsenemy.x + 144), (obj_clubsenemy.y + 125), obj_clovertail_intro)
        tail.damage = damage
        tail.target = target
    }
}
...
else if (type == 48)
{
    if (btimer >= 135 && (!gml_Script_i_ex(849)))
    {
        ...
        bm = gml_Script_instance_create((320 + xmod), 40, obj_ch2_dojo_puzzlebullet_maker)
        bm.grazepoints = grazepoints
        bm.damage = 1
        bm.target = target
        switch roundcount
...
else if (type == 49)
{
    if (made == 0)
    {
        ...
        with (obj_heart)
            color = 0x000001
        gml_Script_instance_create((gml_Script_camerax() + 424), (gml_Script_cameray() + 100), obj_sneo_faceattack)
        with (obj_battlesolid)
            x = (gml_Script_camerax() + 260)
    }
}
else if (type == 50)
{
...
else if (type == 51)
{
    ...
    {
        made = 1
        pip = gml_Script_instance_create((gml_Script_camerax() + 530), (gml_Script_cameray() + 100), obj_sneo_weird_end_pipis)
        pip.damage = damage
        pip.target = target
        pip = gml_Script_instance_create((gml_Script_camerax() + 448), (gml_Script_cameray() + 168), obj_sneo_weird_end_pipis)
        pip.damage = damage
        pip.target = target
        pip = gml_Script_instance_create((gml_Script_camerax() + 510), (gml_Script_cameray() + 250), obj_sneo_weird_end_pipis)
        pip.damage = damage
        pip.target = target
    }
    ...
    if (btimer >= 100 && btimer < 110)
    {
        ...
        with (obj_spamton_neo_enemy)
            gml_Script_scr_move_to_rememberxy(4)
        global.turntimer = 1
    }
}
