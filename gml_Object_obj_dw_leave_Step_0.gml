...
if (con == 10)
{
    ...
    with (obj_caterpillarchara)
    {
        if (visible == true)
        {
            me = gml_Script_scr_dark_marker_depth(x, y, 510000, sprite_index)
            visible = false
            if (name == "ralsei")
            {
...
if (stopcon == 1)
{
    if (global.interact == 0 && obj_mainchara.y <= (obj_markerX.y - 8))
    {
        ...
        gml_Script_msgsetloc(0, "\\EN* Come back when you've finished your homework!/", "obj_dw_leave_slash_Step_0_gml_71_0")
        gml_Script_scr_anyface_next("susie", "K")
        gml_Script_msgnextloc("\\EK* (What's the point of having a room if you can't slack off in it?)/%", "obj_dw_leave_slash_Step_0_gml_73_0")
        gml_Script_d_make()
    }
}
if (stopcon == 2 && (!gml_Script_d_ex()))
{
    stopcon = 1
    global.interact = 0
}
