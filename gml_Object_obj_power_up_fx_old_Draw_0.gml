...
var __flash = lerp(((timer + 15) >= poweruptime ? 0.9 : __alpha), 1, ((flashtimer / flashspeed) * pi))
1
...
if (intensity == 2)
{
    if ((timer + 15) >= poweruptime && (timer + 15) <= (poweruptime + 30))
    {
        ...
        var _circlesize = lerp(0.75, 1, gml_Script_scr_ease_out(_progress, 5))
        var _circlewidth = (_circlesize * (1 - gml_Script_scr_ease_out(min((_progress * 3), 1), 5)))
        (1 - gml_Script_scr_ease_in(_progress, 4))
        gml_Script_scr_draw_circle_width_qb(centerX, centerY, ((_circlesize * 80) - (_circlewidth / 2)), (_circlewidth * 80), 36)
        1
    }
}
else if (intensity == 3)
