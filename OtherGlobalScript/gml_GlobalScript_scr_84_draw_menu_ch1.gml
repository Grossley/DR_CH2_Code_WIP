...
for (var i = 0; i < length; i += 3)
{
    ...
    if (type == "[group]")
        name = (("[ " + name) + "... ]")
    gml_Script_scr_84_draw_text_outline_ch1(xx, yy, (prefix + name))
    yy += vspacing
    if (func_depth < menu_depth && ndx == selection_indices[func_depth])
        yy = gml_Script_scr_84_draw_menu_ch1(item, (xx + 20), yy, vspacing, selection_indices, (func_depth + 1), menu_depth)
}
return yy;
